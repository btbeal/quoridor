...
class Board():
    ...
    def get_adjacent_wall(self, wall: Wall):
        if wall.is_vertical:
            coordinate_to_search = self._get_new_position(curr_position=wall.position, direction='down', distance=DISTANCE)
        else:
            coordinate_to_search = self._get_new_position(curr_position=wall.position, direction='right', distance=DISTANCE)
    ...
    def get_objects_around_node(
        ...
        proximal_objects = {}
        direction_list = [direction for direction in iter(Direction) if direction != exclude_direction and direction != pygame.K_a]
    ...
    def check_viable_path(self, player_index: int, player_center):
        ...
        while len(stack) > 0:
            ...
            if vertex not in seen:
                ...
                neighbors = self.get_objects_around_node(current_vertex, group=self.nodes)
                walls_between_neighbors = self.get_objects_around_node(current_vertex, group=self.walls)
                valid_neighbors = []
